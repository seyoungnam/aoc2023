...
var numToDirc = map[string]string{
	"0": "R",
	"1": "D",
	"2": "L",
	"3": "U",
...
func loadFile(fileName string) (plan []string, num []int) {
	...
	for scanner.Scan() {
		...
		hexMeter := string(arr[2][2:7])
		meter, _ := strconv.ParseInt(hexMeter, 16, 64)
		dir := string(arr[2][7])
		plan = append(plan, numToDirc[dir])
		// n, _ := strconv.Atoi(meter)
		num = append(num, int(meter))
	}
	return plan, num
}
